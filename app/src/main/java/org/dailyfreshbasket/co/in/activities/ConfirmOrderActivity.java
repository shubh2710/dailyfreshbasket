...
import org.dailyfreshbasket.co.in.adapters.CustomSpinnerAdapter;
...
import static org.dailyfreshbasket.co.in.informations.GetDomain.KEYS.URL_PLACEORDER;
import static org.dailyfreshbasket.co.in.informations.SheardPrefsKeys.KEYS.PREF_FILE_NAME;
import static org.dailyfreshbasket.co.in.informations.SheardPrefsKeys.KEYS.PREF_KEY_ADDRESS;
...
public class ConfirmOrderActivity extends AppCompatActivity implements View.OnClickListener, getResponce, AdapterView.OnItemSelectedListener {
    ...
    private TextView total,line,land,city,mobile,message,name,tvRbDay,tvRbNight;
    ...
    public void setup(){
        ...
        tvRbDay = findViewById(R.id.tv_rb_day);
        tvRbNight = findViewById(R.id.tv_rb_night);
...
//                        if(currentHour>8 && currentHour<12){
                        if(currentHour>=8){
                            ...
                            tvRbDay.setText("First shift available tomorrow");
                            Toast.makeText(ConfirmOrderActivity.this,"It will deliver on First shift tomorrow",Toast.LENGTH_LONG).show();
                            Intshift = 0;
                        }else{
                            shift.check(R.id.rb_confirm_dayshift);
                            tvRbDay.setText("First shift available today");
                            Toast.makeText(ConfirmOrderActivity.this,"It will deliver on First shift today",Toast.LENGTH_LONG).show();
                            Intshift = 0;
                        }
                        checkCanPlaced();
                        break;
                    case R.id.rb_confirm_nightshift:
                        if(currentHour>=0&& currentHour<=16){
                            ...
                            tvRbNight.setText("Second shift available today");
                            Toast.makeText(ConfirmOrderActivity.this,"It will deliver on second shift today",Toast.LENGTH_LONG).show();
                        }else{
                            Intshift=1;
                            shift.check(R.id.rb_confirm_nightshift);
                            tvRbNight.setText("Second shift available tomorrow");
                            Toast.makeText(ConfirmOrderActivity.this,"It deliver on second shift tomorrow.",Toast.LENGTH_LONG).show();
                        }
                        checkCanPlaced();
                        break;
                        default:
        ...
        super.onActivityResult(requestCode, resultCode, data);
            if(resultCode==RESULT_OK){
               ...
               String city=data.getStringExtra("city");
                ...
                this.charge.setText("Rs "+charge);
                int am = Total + charge;
                netTotal.setText("Rs "+am);
    ...
    public void confirmAndplace(){
        if(data.size()>0){
            if(Intshift != -1 && IntMethod==0 ){
                pd=ProgressDialog.show(this, "Placing Order", "wait...");
                pd.setCancelable(true);
                String uid=readPrefes(this,PREF_KEY_UID,"");
                Log.d("UID",uid);
...
//                    product.put("name",p.getPname());//edited also added get and set in PlaceOrderListModel
        ...
        case 2:
            ...
            if(result){
                ...
                if(data.size()>0){
                    ...
                    change.setText("Change");
                    int am = (Total+a.getCharge());
                    netTotal.setText("Rs "+am);
                }else{
                    change.setText("ADD");
                    charge.setText("Rs 0");
                    netTotal.setText("Rs "+(Total));
